...
CSV_FILE = f"../data/auto_wifi_packets_{ts_file}.csv"
...
logger.info(f"Auto Wi-Fi monitor sniffing started for {DURATION} seconds")
...
try:
    sniff(
        ...
        prn=handle,
        store=False,
        timeout=DURATION   #หยุดอัตโนมัติ
    )

except KeyboardInterrupt:
    logger.info("Stopping auto sniffer (manual interrupt)...")

finally:
    csvfile.close()
    logger.info("Auto sniffing finished. CSV file saved.")
